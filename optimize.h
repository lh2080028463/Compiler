...
public:
    ...
    void addsign(int n,QString A,QString typ);   //把标记A添加到节点n上，顺便删除无用标记
    void makeleaf(QString B,QString typ);   //构造叶子节点
    void delnode(int n);     //删除id为n的节点
    //void blockqua(int block,fstream& File);   //生成block块中优化后的四元式
    //void optqua(int block_num,fstream& File);    //优化四元式
